...
if ( version_compare( get_bloginfo( 'version' ), WPCW_MIN_WP_VERSION, '<' ) ) {
    add_action(
        ...
        <div class="error is-dismissible">
            <p><?php printf( 'WP Canje Cupón WhatsApp requiere WordPress %s o superior.', WPCW_MIN_WP_VERSION ); ?></p>
        </div>
            <?php
        }
    );
    ...
    if ( version_compare( WC_VERSION, WPCW_MIN_WOOCOMMERCE_VERSION, '<' ) ) {
        add_action(
            ...
            function () {
                ?>
            <div class="error is-dismissible">
                <p><?php printf( 'WP Cupón WhatsApp requiere WooCommerce %s o superior.', WPCW_MIN_WOOCOMMERCE_VERSION ); ?></p>
            </div>
                <?php
            }
        );
    ...
    if ( file_exists( $file_path ) ) {
        ...
        add_action(
            ...
            function () use ( $file ) {
                ?>
            <div class="error is-dismissible">
                <p><?php printf( 'WP Canje Cupón WhatsApp: Archivo core no encontrado: %s', $file ); ?></p>
            </div>
                <?php
            }
        );
...
foreach ( $public_files as $file ) {
    ...
    if ( file_exists( $file_path ) ) {
        require_once $file_path;
    } elseif ( class_exists( 'WPCW_Logger' ) ) {
    ...
    if ( version_compare( ELEMENTOR_VERSION, WPCW_MIN_ELEMENTOR_VERSION, '>=' ) ) {
        ...
        add_action(
            ...
            function () {
                ?>
            <div class="error is-dismissible">
                <p><?php printf( 'WP Canje Cupón WhatsApp requiere Elementor %s o superior para sus widgets.', WPCW_MIN_ELEMENTOR_VERSION ); ?></p>
            </div>
                <?php
            }
        );
...
if ( is_admin() ) {
    ...
    // Load debug files only in debug mode
...
function wpcw_public_enqueue_scripts_styles() {
    ...
    if ( $post instanceof WP_Post ) {
        $should_load = (
            ...
            has_shortcode( $post->post_content, 'wpcw_canje_cupon' ) ||
