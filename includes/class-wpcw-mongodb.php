...
class WPCW_MongoDB {
    ...
    private function connect() {
        try {
            ...
            if (class_exists('WPCW_Logger')) {
                WPCW_Logger::log('error', 'MongoDB connection error', array(
                    'error' => $e->getMessage()
                ));
            }
        }
        ...
        try {
            ...
            foreach ($canjes as $canje) {
                ...
                $collection->updateOne(
                    ['numero_canje' => $canje['numero_canje']],
                    ['$set' => $canje],
                    ['upsert' => true]
                );
            }
            ...
            if (class_exists('WPCW_Logger')) {
                WPCW_Logger::log('error', 'MongoDB sync error', array(
                    'error' => $e->getMessage()
                ));
            }
            return false;
    ...
    public function export_data($format = 'json', $collection = 'canjes') {
        try {
            ...
            if (class_exists('WPCW_Logger')) {
                WPCW_Logger::log('error', 'Export error', array(
                    'error' => $e->getMessage(),
                    'format' => $format
                ));
            }
