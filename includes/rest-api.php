...
function wpcw_handle_redemption_confirmation_request( WP_REST_Request $request ) {
    ...
    $new_coupon_description = sprintf( __( 'Cupón de canje para "%1$s". Solicitud Nro: %2$s.', 'wp-cupon-whatsapp' ), $original_coupon_post->post_title, $numero_canje_original );
    ...
    $page_content .= '<p>' . sprintf( esc_html__( 'El canje para "%1$s" (Solicitud Nro: %2$s) ha sido confirmado.', 'wp-cupon-whatsapp' ), esc_html( $original_coupon_post->post_title ), esc_html( $numero_canje_original ) ) . '</p>';
    ...
    if ( ! empty( $cliente_whatsapp_cleaned ) ) {
        ...
        $page_content              .= '<li>' . sprintf( wp_kses_post( __( 'Puedes enviárselo directamente por WhatsApp haciendo clic aquí: <a href="%1$s" target="_blank">Enviar Código por WhatsApp al Cliente</a> (Número: %2$s)', 'wp-cupon-whatsapp' ) ), esc_url( $whatsapp_link_to_client ), esc_html( $cliente_whatsapp ) ) . '</li>';
    } else {
