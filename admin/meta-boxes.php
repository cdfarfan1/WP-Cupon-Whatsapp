 ...
 */
function wpcw_save_coupon_metabox_fields( $post_id, $coupon ) {
    // $coupon might not always be passed or be useful, $post_id is key.
    // 1. Verificar Nonce
...
function wpcw_admin_enqueue_metabox_scripts( $hook ) {
    ...
    if ( ( 'post.php' == $hook || 'post-new.php' == $hook ) && isset( $post_type ) && 'shop_coupon' === $post_type ) {
        ...
        $plugin_url     = defined( 'WPCW_PLUGIN_URL' ) ? WPCW_PLUGIN_URL : plugin_dir_url( __DIR__ ); // Fallback for URL
